...
class Carga(db.Model):
    ...
    # Relacionamentos com motorista e veículo
    motorista_id = db.Column(db.Integer, db.ForeignKey('usuarios.id', ondelete='CASCADE'))
    veiculo_id = db.Column(db.Integer, db.ForeignKey('veiculos.id', ondelete='CASCADE'))
    motorista = db.relationship('Usuario', foreign_keys=[motorista_id], backref=db.backref('cargas_motorista', lazy=True))
    veiculo = db.relationship('Veiculo', backref=db.backref('cargas', lazy=True))
