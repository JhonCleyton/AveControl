...
window.DocumentosModule = window.DocumentosModule || (function() {
        ...
        .then(response => {
            ...
            return response.text().then(text => {
                try {
                    ...
                    // Se falhar, mostrar o texto recebido para debug
                    console.error('Texto recebido não é JSON válido:', text.substring(0, 500));
    ...
    function adicionarDocumento(cargaId, formData) {
        ...
        if (!cargaId) {
            console.error('ID da carga não fornecido');
            return Promise.reject(new Error('ID da carga não fornecido'));
        }

        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        
        uploadInProgress = true;
        // Verificar se o elemento existe antes de modificar
